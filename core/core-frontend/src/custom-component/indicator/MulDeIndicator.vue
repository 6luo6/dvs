...
const getResultName = index => {
  if (view.value?.yAxis?.length) {
    const axis = view.value.yAxis[index]
...
const formattedResult = (value, index) => {
  let _result = value
  ...
  // 格式化
  if (view.value.yAxis && view.value.yAxis.length > 0 && view.value.yAxis[index].formatterCfg) {
    return valueFormatter(_result, view.value.yAxis[index].formatterCfg)
  }
  return _result
}
...
const renderChart = async view => {
  ...
  if (chart.customAttr) {
    ...
    //多项
    let mulItem = chart.customAttr.mulItem
    if (mulItem) {
      flexBoxStyle.value['rowGap'] = mulItem.rowGap + 'px'
      flexBoxStyle.value['columnGap'] = mulItem.columnGap + 'px'
      if (mulItem.columnCount) {
        contentStyle.value['width'] = `calc((100% - ${
          mulItem.columnGap * mulItem.columnCount
        }px) / ${mulItem.columnCount})`
      }
    }
  }
}
...
<template>
  <div :style="flexBoxStyle">
    <div :style="contentStyle" v-for="(item, index) in chartData.series">
      <div>
        <span :style="indicatorClass">{{ formattedResult(item.data[0], index) }}</span>
        <span :style="indicatorSuffixClass" v-if="showSuffix">{{ suffixContent }}</span>
      </div>
      <div v-if="indicatorNameShow">
        <span :style="indicatorNameClass">{{ getResultName(index) }}</span>
      </div>
    </div>
  </div>
</template>
